...
## Ten-fix round (2026-08-10, evening) — chat + sessions hardening

Goal: `/tmp/cockpit-goal-ten-fixes.txt` (pi-goal). Each fix shipped with regression coverage, runtime evidence, and a taste-review checkpoint; per-fix screenshots saved as `/tmp/cockpit-<fix>.png`. The taste-review skill was upgraded this round: the Claude helper now drives the emulator itself (install/launch/tap/swipe/screencap/gradle) instead of reviewing static images (skill at `~/.agents/skills/taste-review`, symlinked to `~/.claude/skills`).

- [x] Fix 1 — composer Attach/Send overlap (`61d9403`). Root cause: M3 1.4.0 trailingIcon stacks sibling children. Fix: explicit `Row` wrap in the composer trailing area. Regression: `attachAndSendIconsNeverOverlap` (`ChatComposerKeyTest`). Evidence: `/tmp/cockpit-1.png` (+ `/tmp/cockpit-1-before.png`), vision + Claude verified.
- [x] Fix 2 — assistant text renders as markdown (`28c71ea`). `com.mikepenz:multiplatform-markdown-renderer-m3:0.41.0`, themed to the dark M3 design (`AssistantMarkdown.kt`): mono 13sp code, `surfaceContainerHigh` code backgrounds, blockquote bar `outlineVariant`, h1–h6 capped scale, links primary `#5B8CFF` underline. User bubbles stay plain. Evidence: `/tmp/cockpit-2.png` (heading/bold/lists/blockquote/link/code all verified in-app). Unblocking pre-existing fix: session-catalog DTO decoded the wrong wire shape (`sessionId` vs bridge `id`), so Sessions was always empty — `ef99656` aligns the DTO to the bridge and adds `ModelsTest` coverage.
- [x] Fix 3 — quiet tool-call chips (`fa9bd9d`). Calls are one-line no-fill rows (`▸ name` + meaningful arg, 12sp dim mono), results are indented faint-fill cards (`surfaceVariant`@0.3, 2-line preview), tool entries group at 4dp while prose gets 14dp air; errors keep an explicit `▸ tool (error)`. Regression: `toolCallChipShowsCollapsedByDefaultAndExpandsOnTap`, `detailsToggleExpandsToolCallChip` (`ChatListTest`), `ChatFormatTest` arg-preference cases. Evidence: `/tmp/cockpit-3.png`.
- [x] Fix 4 — swipeable session rows (`d95d4c1`). `HistoryRow` gains swipe-to-reveal via `anchoredDraggable` (Closed/Open anchors, horizontal-only so it never fights row taps, list scroll, or the search field): a left swipe slides the card and exposes rename/pin/archive + close (active) or delete (inactive) as an outlined-icon bar that sizes itself from the card (`matchParentSize`; LazyColumn items measure with unbounded height). Tapping a revealed button closes the reveal and fires the action; tapping an open row just closes it. Regression: 4 new `HistoryScreenTest` tests (swipe reveals rename, covered actions not tappable, swipe reveals delete, revealed bar matches row height). Evidence: `/tmp/cockpit-4.png`. Taste agent (drove the emulator itself) caught and fixed the 24dp bar-collapse bug, unified icon weights to `Icons.Outlined.*`, and added a stable test-tag key.
- [x] Fix 5 — review 403 (live agent workspaces, least-privilege narrowed) (`8c2c5ea`). Review roots join `COCKPIT_REPO_ROOTS` with live session workspaces, each session cwd resolved to its git repo root (`git rev-parse --show-toplevel`) so a cwd of `$HOME` or a scratch dir grants nothing — the whole-home-directory grant found in taste review is closed, and `$HOME`/`/etc` now hard-403 with an actionable reason ("review a running agent's workspace, or add the path to COCKPIT_REPO_ROOTS"). `BridgeClient` surfaces the bridge's `error` reason on non-2xx instead of a bare status line. The Sessions swipe bar gains a first action "Review" (`Icons.Outlined.Code`, the Review tab's own nav glyph — taste agent caught the RateReview/Rename pencil collision) that loads the row's workspace cwd into the Review tab via a hoisted `ReviewViewModel`. Tests: `gitRepoRoot` unit tests + extra-roots coverage in `review.test.ts` (bridge 152/152), review-action swipe assertion (emulator 54/54). Evidence: `/tmp/cockpit-5.png` (swipe → Review loading `agents-mobile` branch main, bridge running without `COCKPIT_REPO_ROOTS`) and `/tmp/fix5-final-403.png`. Live curl verified: workspace 200, `$HOME`/`/etc`/artifacts 403.
- [x] Fix 6 — provider-grouped model selection (`597e8ff`). The model picker renders one `ProviderHeader` per provider (mono `labelSmall`, letter-spaced, `onSurface` — a contrast step above the mono key line it presides over) followed by that provider's models in ranked order; `groupBy` preserves encounter order so the top-ranked match still leads, and every model appears under exactly one provider. Filters (reasoning/context/thinking) still narrow across providers; groups with no matches vanish, no orphan headers. The count right-aligns with the star column so it reads as a group counter (taste agent). Row testTags use the provider-qualified key so a model id shared by two providers still resolves to a single node. Regression: `modelPickerGroupsModelsUnderProviderHeaders` (`NewSessionSheetTest`). Evidence: `/tmp/cockpit-6.png` (top frame: `openai-codex · 7` header) and `/tmp/fix6-boundary.png` (group boundary into `deepseek · 2`). Emulator 55/55, unit 96/96.
- [x] Fix 7 — resume semantics, user-decided (`9215d40`). Asked the user with options: (1) workspace semantics = keep the current fresh-workspace per resume (`pi --session <path>` — `--resume/-r` is an interactive TUI picker that cannot be driven headlessly; both end at the same `SessionManager.open(path)`); (2) resume cwd = trust the cwd recorded in the session file (the user already ran an agent there — same least-privilege reasoning as fix 5), falling back to the session store root then the session-file directory when it no longer exists. This unblocks resuming sessions run outside `$HOME` (previously a 400). Tests: `sessions.test.ts` covers outside-`$HOME` cwd + fallback; bridge 154/154. Evidence: `/tmp/cockpit-7.png` — the `/tmp`-cwd markdown demo session resumes in-app and opens its chat (was a 400 banner); live curl 201, new workspace launches in `/tmp`. Backend-only commit; taste gate satisfied by the live in-app resume walk.
- [x] Fix 14 — image attachment 401 in real use (`add5760`). The user's attach→send failed with `bridge 401: unauthorized`; root cause was an app-side header corruption, not the bridge auth gate: `uploadAttachment`'s `Authorization` header carried the literal string `Bearer ${'$'}{token()}` (corrupted in `2b26111`), so every upload was rejected while GETs used the real token. Only the real app flow could expose it — curl with a valid token could not. Also fixed on the path: the generic POST JSON pre-parse 400'd binary uploads before the attachments route (skipped for `/api/attachments`), and `storeAttachment` mkdirs the uploads dir (was 500 when absent). Regression: `BridgeClientUploadTest` (MockWebServer) pins the header + body and was verified to fail on the corrupted header; `server.test.ts` raw-binary-body test. Verification loop upgraded per the user's "test this better": an androidTest drove the REAL `BridgeClient` against the scratch bridge (reproduced the 401 deterministically), then the full in-app attach→send walk stored the 713-byte image and the pending bubble carried the `@path` prompt. Evidence: `/tmp/cockpit-14.png`.
- [x] Fix 13 — review 403 on completed sessions (`34b1cb2`). Reviewing a completed session's repo still 403'd in real use because only LIVE agent cwds were implicit review roots. The review route now also resolves catalog session cwds through `gitRepoRoot` (failure-tolerant), so a completed session's repo is reviewable while non-repo cwds and `$HOME` still contribute nothing. Regression: `server.test.ts` catalog-review test (completed session cwd=<temp repo> → 200; rewritten to `/home` → 403).
- [x] Deployment gate — production bridge ran stale `dist/` (incident). The user's `bridge 403: path outside allowed repo roots` persisted AFTER fixes 5/13 because the phone talks to systemd `cockpit-bridge.service` (port 8737, `~/.config/cockpit/config.json`, exposed via `tailscale serve`), which runs the COMPILED `dist/` — built 17:41, before the fixes landed — while every source-level test (bridge 152/152, scratch-bridge curl, in-app walk) verified the fixed source. `dist/server.js` had zero occurrences of `sessionWorkspaceRoots`; the live endpoint hard-403'd even with the real token. No test can catch a stale deployment; the fix is a deploy gate. Added `bridge/scripts/check-deployed.mjs` + npm scripts (`npm run deploy` = build + restart + gate; `npm run check:deployed` fails if dist < newest src, service started before the dist build, or the real bridge fails health). Rebuilt + restarted, verified: real endpoint 200 for agents-mobile, `$HOME` still 403 (least-privilege intact), HTTPS via tailscale 200, and the review flow driven live ON THE PHONE (Board swipe → Review loads `agents-mobile · branch main` with no 403 banner). docs/dev-workflow.md gained a "Deploying bridge changes" section.
- [x] Fix 9 — text copy support (`fed4737`). Chat message prose (assistant markdown + user bubbles) is wrapped in a `SelectionContainer`, so a long press raises the system toolbar and Copy works — the core ask. The taste agent (driving the emulator itself) proved that wrapping LIST rows the same way is broken: each short ellipsized line becomes its own tiny container whose toolbar can't be dismissed by tapping outside (it stuck and covered content, even stealing a tap and ejecting to Google Search), scrolling re-anchors the selection onto a different row's text and Copy returns the wrong string, and in the model picker the long-press lost to the row's clickable and silently switched the conversation's model. Those three files were reverted; the copy want ships as explicit actions instead (taste-recommended): Board cards gain a third swipe action 'Copy' (ContentCopy glyph) that copies the agent's cwd, and Sessions rows gain a 'Copy path' menu item that copies the session's cwd — both with haptic + a 'Copied path' toast. Tool chips stay unselectable so tap-to-expand is untouched. Regression: selection tests count the two selection-handle popups that only exist on a real selection (a plain unwrapped Text yields zero) plus a tool-chip guard; copy tests read the app's REAL `ClipboardManager` after tapping the action. Emulator 68/68, unit suite green; live-verified swipe → Copy on the Board (toast 'Copied path' via vision pane). Evidence: `/tmp/cockpit-9-reveal.png`, `/tmp/cockpit-9-toast.png`, `/tmp/fix9-taste-*.png`.
- [x] Fix 15 — swipe-to-reveal on Board cards (`f4f0cc7`). Board cards swipe left to reveal a 2-action icon bar (Review with the Code glyph steering the hoisted `ReviewViewModel` to the agent's cwd + Review tab; Close stopping the agent's pane), mirroring the Sessions rows' `anchoredDraggable` pattern: `matchParentSize` bar sizing (LazyColumn items measure with unbounded height), outlined glyphs at one weight, horizontal-only anchoring so vertical board scrolling is untouched, tap on an open card closes the reveal without navigating. The taste agent (driving the emulator itself) caught that a one-tap Close would kill a live pane with no second thought — a horizontal swipe is easy to trigger while scrolling, and every board card is a running agent — so Close now asks first via `ConfirmDialog` extracted out of `HistoryScreen` into `ui/components` (internal, shared; same deliberate second tap wherever "stop" is reached from). Close keeps `onSurfaceVariant` tint, not error red: Sessions reserves error for irreversible Delete, and Close preserves the transcript. Regression: `BoardViewModelTest` (control POST + bridge-error surfacing) and 5 `BoardScreenTest` swipe tests (review fires with the agent cwd without opening the card, close requires the confirm, cancel keeps the agent running, covered action not tappable, action bar matches card height). Unit 99/99, emulator 63/63; live-verified swipe → Review loading the `agents-mobile` workspace overview. Evidence: `/tmp/cockpit-15-reveal.png`, `/tmp/cockpit-15-review.png`, `/tmp/fix15-taste-*.png`.
...
- [x] Stop/restart, permission denial, network loss, reboot, and background constraints — implemented and unit-covered: the service stops itself without a connection, START_STICKY restarts it, the reply action is gated to blocked events, `MonitoringStoreTest` covers the lifecycle, `SettingsScreenTest` covers the opt-in UI. Live end-to-end validation COMPLETED: a synthetic blocked event published to the real self-hosted ntfy topic reached the foreground service, produced the 'needs you' notification, and tapping it deep-linked to the exact session (`/tmp/cockpit-deeplink-final.png`). This validation caught a REAL production bug: ntfy silently drops custom JSON fields, so the `paneId` deep link never reached the app; the bridge now publishes the link in ntfy's documented `click` field and the app uses it (falling back to paneId). Fix committed with updated notify tests and the notification-intent unit test.
