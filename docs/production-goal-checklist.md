...
- [x] Slash commands are functional and discoverable: typing `/` opens autocomplete for pi built-ins, prompt templates, extension commands, and trusted installed skills, with description/source rows, ranked filtering, keyboard/touch completion, and a distinct second submit. The bridge uses the pinned pi 0.84.1 `DefaultResourceLoader` and `ProjectTrustStore`, applies pi's package/filter/precedence/duplicate-command rules, fails closed on invalid trust data, caches catalogs for 30 seconds, bounds and validates cwd/catalog reads, and delivers each command through one atomic `pane.send_input` operation so `/compact` cannot become split terminal input. Loading, empty, error, no-match, long-list, blocked-session routing, stale catalog responses, agent disappearance, package/conventional skills, prompt/extension commands, punctuation in command names, and unsafe input are covered by bridge, unit, and emulator instrumentation tests. Evidence: `bridge/src/pi/commands.ts`, `bridge/src/server.ts`, `SlashCommandSearch.kt`, `SlashCommandMenu.kt`, `ChatViewModel.kt`, 97 passing bridge tests, Android unit/build gates, 3 passing `SlashCommandMenuTest` emulator tests, and directly inspected emulator screenshots `/tmp/cockpit-e2e/54-slash-menu-final.png`, `55-slash-menu-final-all.png`, `56-slash-built-in-final.png`, and `57-slash-command-sent.png`.
