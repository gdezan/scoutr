...
class HistoryFormatTest {
    ...
    fun historyLadder_switchesToDateAfterSevenDays() {
        ...
        assertEquals("now", dev.cockpit.app.ui.screens.relativeTime((now - 5_000).toDouble(), nowMs = now))
        assertEquals("12m", dev.cockpit.app.ui.screens.relativeTime((now - 12 * 60_000).toDouble(), nowMs = now))
        assertEquals("2h", dev.cockpit.app.ui.screens.relativeTime((now - 2 * 3_600_000).toDouble(), nowMs = now))
        assertEquals("3d", dev.cockpit.app.ui.screens.relativeTime((now - 3 * 86_400_000).toDouble(), nowMs = now))
        // Past a week the timestamp renders as a "MMM d" date (UTC-pinned):
        // now - 40d = 2023-10-05T22:13:20Z → Oct 5 in every timezone.
        assertEquals(
            "Oct 5",
            dev.cockpit.app.ui.screens.relativeTime((now - 40L * 86_400_000L).toDouble(), nowMs = now),
        )
    }
