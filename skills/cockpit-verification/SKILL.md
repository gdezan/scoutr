...
description: Verify Cockpit changes in two phases: cheap targeted checks during implementation/review, then emulator/integration/E2E/runtime acceptance only after reviews are resolved and the code is otherwise final. Use before committing bridge/Android work or when runtime proof is requested.
---
...
3. **Review:** simplify/self-review and run `skills/cockpit-review/SKILL.md`; fix all accepted findings.
...
4. `skills/cockpit-review/SKILL.md` has completed;
...
```bash
marker="__COCKPIT_CHECK_$(date +%s%N)__"
...
adb shell am force-stop dev.cockpit.app
...
```bash
adb exec-out screencap -p > /tmp/cockpit-<name>.png
```

Inspect screenshots directly when the active model supports vision; otherwise use `skills/cockpit-vision/SKILL.md`, which delegates through Herdr lifecycle state. Save only evidence that answers a concrete acceptance question.
...
Copy to `~/.pi/agent/skills/cockpit-verification/` (or `~/.agents/skills/cockpit-verification/`) to make it loadable from any repo.
