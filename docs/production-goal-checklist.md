...
- [ ] Stop/restart, permission denial, network loss, reboot, and background constraints handled. Service stops without a connection; START_STICKY covers restarts; reply action gated to blocked events. Killed-app/background validation and a notification deep-link screenshot still require a live blocked event + ntfy.
