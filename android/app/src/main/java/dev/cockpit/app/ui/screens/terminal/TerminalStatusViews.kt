...
import dev.cockpit.app.state.TerminalUiState
...
internal fun TerminalOverlay(
    ...
    when (val c = state.connection) {
        is dev.cockpit.app.state.TerminalConnectionState.Connecting,
        is dev.cockpit.app.state.TerminalConnectionState.Reconnecting,
        -> ConnectingOverlay(c is dev.cockpit.app.state.TerminalConnectionState.Reconnecting)
        is dev.cockpit.app.state.TerminalConnectionState.Failed -> FailedOverlay(
            message = c.message,
            retryable = c.retryable,
            onRetry = onRetry,
        )
        is dev.cockpit.app.state.TerminalConnectionState.Unsupported -> UnsupportedOverlay(c.explanation)
        dev.cockpit.app.state.TerminalConnectionState.Closed -> EmptyOverlay(paneCount, onRetry)
        is dev.cockpit.app.state.TerminalConnectionState.Idle,
        is dev.cockpit.app.state.TerminalConnectionState.Ready,
        -> Unit
    }
...
internal fun TerminalStatusChip(state: TerminalUiState) {
    val (label, color) = when (val c = state.connection) {
        ...
        is dev.cockpit.app.state.TerminalConnectionState.Failed,
        is dev.cockpit.app.state.TerminalConnectionState.Unsupported,
        -> "Unavailable" to MaterialTheme.colorScheme.error
        is dev.cockpit.app.state.TerminalConnectionState.Closed -> "Closed" to MaterialTheme.colorScheme.onSurfaceVariant
        is dev.cockpit.app.state.TerminalConnectionState.Idle -> "Idle" to MaterialTheme.colorScheme.onSurfaceVariant
    }
