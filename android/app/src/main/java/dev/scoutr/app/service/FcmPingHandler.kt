 ...
 * while Scoutr is already on screen; a `done` ping shows “Finished” when
 * the agent finishes, also dropped while foregrounded.
 */
class FcmPingHandler(
    ...
    private suspend fun handleDone(paneId: String) {
        if (isForegrounded()) return
        for ((attempt, waitMs) in DONE_FETCH_RETRY_DELAYS_MS.withIndex()) {
            ...
            try {
                ...
                if (attempt == DONE_FETCH_RETRY_DELAYS_MS.lastIndex) {
                    presenter.showDegradedDone(paneId)
                }
            }
        }
    }
