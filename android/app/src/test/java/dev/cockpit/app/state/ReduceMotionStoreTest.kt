...
import dev.cockpit.app.ui.motion.toFeedbackType
...
class MotionSpecTest {
    ...
    fun itemSpecCollapsesToZeroUnderReduceMotion() {
        val normal = CockpitMotion.itemSpec(false)
        val reduced = CockpitMotion.itemSpec(true)
        assertNotEquals(normal, reduced)
    }

    @Test
    fun overlaySpecIsZeroUnderReduceMotion() {
        assertEquals(0, (CockpitMotion.overlaySpec(true) as androidx.compose.animation.core.TweenSpec<*>).durationMillis)
        assertEquals(CockpitMotion.DURATION_OVERLAY, (CockpitMotion.overlaySpec(false) as androidx.compose.animation.core.TweenSpec<*>).durationMillis)
    }
