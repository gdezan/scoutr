...
**Measured (2026-08-24, dev machine, not a device claim):** a synthetic 14.0 MiB / 8000-entry pi transcript, warm page cache, median of 7 runs under Node 26. The full `readTranscript` this path replaced: **53.8 ms**. The bounded composite that replaces it: display read **0.3 ms** + exact state read **12.3 ms** + ask-record scan **9.5 ms** ≈ **22 ms**, and **0.5 ms** once the derived state is memoized. The record-line filter is what makes that a win rather than a regression: before it, the exact state read alone cost 50.2 ms, so the composite would have been slower than the full read it replaced. These are wall-clock host timings; the larger effect — not normalizing 8000 entries into objects to send 50 — was not measured as heap. Structural proof that the display read is bounded is in `session-read.test.ts` ("serves the initial page from the bounded tail window" asserts the only display read is `{ tail: 6 }`), with truthful history, model/thinking, and question coverage beside it.
