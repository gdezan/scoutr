...
class CockpitMonitorServiceTest {
    ...
    fun setUp() {
        ...
        context.getSharedPreferences("cockpit_monitoring", Context.MODE_PRIVATE)
            .edit().clear().commit()
    ...
    fun tearDown() {
        ...
        RuntimeEnvironment.getApplication()
            .getSharedPreferences("cockpit_monitoring", Context.MODE_PRIVATE)
            .edit().clear().commit()
        server.shutdown()
    }
    ...
    fun pollOnce_readsCursorThenAdvancesItPerMessage() {
        ...
        runBlocking {
            CockpitMonitorService.pollOnce(
                client, store,
                baseUrl(), "topic",
            ) { notified.add(it) }
        }
    ...
    fun pollOnce_skipsPaneLessMessagesButStillAdvancesTheCursor() {
        ...
        val notified = mutableListOf<NtfyMessage>()
        runBlocking {
            CockpitMonitorService.pollOnce(client, store, baseUrl(), "topic") {
                notified.add(it)
            }
        }
        assertEquals("m2", store.ntfyCursor)
