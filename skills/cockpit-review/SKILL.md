---
name: cockpit-review
description: Run a fresh independent Pi review of Cockpit's current uncommitted changes before committing. Delegation is orchestrated through Herdr lifecycle state; use the global herdr-agent-delegation skill.
---

# Cockpit pre-commit review
...
Done when the review settled successfully, every concrete finding has been resolved or consciously dismissed, and no further review-driven edits are planned. At that point hand off to `cockpit-verification` for the **final** runtime acceptance phase. Do not review again after a successful final acceptance pass unless that pass exposed a defect that required code changes.
